## GroupProject – экономическая игра на Django
...
В каталоге `media/docum_imgs` хранятся скриншоты, которые можно использовать для наглядной документации (например, в README на GitHub или wiki проекта):

- **Главное меню** – общий вид стартового экрана игры  
  (см. `media/docum_imgs/main_menu_*.png`)
- **Экран магазина / геймплея** – рабочий день, покупатели, планшет  
  (см. `media/docum_imgs/gameplay_*.png`)
- **Банк и кредиты** – оформление и погашение кредита  
  (см. `media/docum_imgs/bank_*.png`)
- **Казино** – колесо удачи и ставки  
  (см. `media/docum_imgs/casino_*.png`)

Вы можете вставить эти изображения в README на GitHub, используя, например:

```markdown
![Главное меню](media/docum_imgs/main_menu_1.png)
```
